...
func RsaSignPSS(key *rsa.PrivateKey, h crypto.Hash, data []byte, opts *rsa.PSSOptions) (string, error) {
	if !h.Available() {
		return "", errors.New("hash function is not available")
	}
	w := h.New()
	w.Write(data)
	sign, err := rsa.SignPSS(rand.Reader, key, h, w.Sum(nil), opts)
	if err != nil {
		return "", err
	}
	return base64.StdEncoding.EncodeToString(sign), nil
...
func RsaVerifyPSS(key *rsa.PublicKey, h crypto.Hash, data []byte, sign string, opts *rsa.PSSOptions) error {
	if !h.Available() {
		return errors.New("hash function is not available")
	}
	sig, err := base64.StdEncoding.DecodeString(sign)
	...
	w := h.New()
	w.Write(data)
	return rsa.VerifyPSS(key, h, w.Sum(nil), sig, opts)
}

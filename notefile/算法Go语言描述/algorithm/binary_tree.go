...
// 根据先序和中序遍历构建二叉树
func PreorderBuild(preorder []int, inorder []int) *TreeNode {
	size := len(preorder)
	if size == 0 {
		return nil
	}
	...
	root.Left = PreorderBuild(preorder[1:i+1], inorder[:i])
	root.Right = PreorderBuild(preorder[i+1:], inorder[i+1:])
	return root
}
func PreorderBuildTree(preorder []int, inorder []int) *TreeNode {
	size := len(preorder)
	if size == 0 {
		return nil
	}
...
// 根据中序和后序遍历构建二叉树
func PostorderBuild(inorder []int, postorder []int) *TreeNode {
	size := len(inorder)
	if size == 0 {
		return nil
	}
	...
	root.Left = PostorderBuild(inorder[:i], postorder[:i])
	root.Right = PostorderBuild(inorder[i+1:], postorder[i:size-1])
	return root
}
func PostorderBuildTree(inorder []int, postorder []int) *TreeNode {
	size := len(inorder)
	if size == 0 {
		return nil
	}
